...
const ref = fs.readFileSync('references/BSC_Quant_Research_editable_saved.html', 'utf8');
const chartRegex = /<div class="sec-label">Hiệu quả hoạt động · theo ngày<\/div>[\s\S]*?<\/svg><\/div>\s*<\/div>\s*<\/div>/;
const chartHtmlMatch = ref.match(chartRegex);
if (chartHtmlMatch) {
    fs.writeFileSync('src/partials/chart.html', chartHtmlMatch[0], 'utf8');
    console.log('[OK] chart.html restored from reference');
}

// 2. Fix UTF-8 issue and layout in share.html
const web = fs.readFileSync('src/templates/web.html', 'utf8');
const cssMatches = [...web.matchAll(/<style>([\s\S]*?)<\/style>/g)];
let shareHtml = `<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="UTF-8">
...
fs.writeFileSync('src/templates/share.html', shareHtml, 'utf8');
console.log('[OK] share.html recreated with UTF-8 encoding');
